...
from .constants import PDSH_LAUNCHER, OPENMPI_LAUNCHER, MVAPICH_LAUNCHER, TORCH_DISTRIBUTED_DEFAULT_PORT
...
def parse_args(args=None):
    ...
    parser.add_argument("--chdir",
                        type=str,
                        default='.',
                        help="change the working directory to ")
    
    parser.add_argument("--env_style",
                        type=str,
                        default='slurm',
                        choices=['slurm', 'torchrun'],
                        help="export environment variable in which style")
...
def main(args=None):
    ...
    if args.job_name is None:
        args.job_name = args.user_script # set job-name as filename 
    ...
    if not resource_pool:
        ...
        if device_count == 0:
            raise RuntimeError("Unable to proceed, no GPU resources available.\n If it is not a training scirpt, why not directly run the cmd?")
    ...
    if not multi_node_exec:
        deepspeed_launch = [
            ...
            "--env_style={}".format(args.env_style),
        ]
        cmd = deepspeed_launch + [args.user_script] + args.user_args
    else:
        args.launcher = args.launcher.lower()
        if args.launcher == PDSH_LAUNCHER:
            runner = PDSHRunner(args, world_info_base64, env_style=args.env_style)

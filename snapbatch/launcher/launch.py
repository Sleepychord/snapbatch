...
def parse_args():
    ...
    parser.add_argument("--env_style",
                        type=str,
                        default='slurm',
                        choices=['slurm', 'deepspeed', 'torchrun'],
...
def main():
    ...
    if args.env_style == 'torchrun':
        current_env["MASTER_ADDR"] = args.master_addr
        current_env["MASTER_PORT"] = str(args.master_port)
        current_env["WORLD_SIZE"] = str(dist_world_size)
        current_env["JOB_NAME"] = str(args.job_name)
        current_env["JOB_ID"] = str(args.job_id)
        ...
        current_env["SLURM_JOB_ID"] = str(args.job_id)
        current_env["MASTER_PORT"] = str(args.master_port)
    ...
    for local_rank in range(0, num_local_procs):
        ...
        if args.env_style == 'torchrun':
            ...
            current_env["SLURM_PROCID"] = str(dist_rank)
            current_env["SLURM_LOCALID"] = str(local_rank)
        else:
            raise NotImplementedError
        # spawn the processes
        if args.training_script.endswith('.sh'):
            cmd = [
                'bash',
                args.training_script,
            ] + args.training_script_args
        else: # python
            assert args.training_script.endswith('.py')
            cmd = [
                sys.executable,
                "-u",
                args.training_script,
